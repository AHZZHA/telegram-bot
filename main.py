...
async def handle_skin_issue(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "لطفاً وضعیتت رو مشخص کن یا اگه خواستی، خودت بنویس:"
    )
    reply_markup = ReplyKeyboardMarkup(keyboard, one_time_keyboard=True, resize_keyboard=True)
    await update.message.reply_text("👇 یکی از گزینه‌ها رو انتخاب کن یا تایپ کن:", reply_markup=reply_markup)
    return MEDICAL_CONDITION

# دارو یا بیماری خاص (متن یا گزینه)
